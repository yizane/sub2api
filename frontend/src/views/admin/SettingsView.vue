    ...
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div v-show="activeTab === 'gateway'" class="space-y-6">
          ...
          <!-- System Default Tier Fallback Chain -->
          <div class="card">
            <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
                {{ t("admin.settings.defaultTierGroupIds.title") }}
              </h2>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ t("admin.settings.defaultTierGroupIds.description") }}
              </p>
            </div>
            <div class="space-y-5 p-6">
              <div
                v-if="defaultTierGroupIdsLoading"
                class="flex items-center gap-2 text-gray-500"
              >
              ...
              <template v-else>
                <div
                  v-if="defaultTierGroupIdsLoadFailed"
                  class="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 dark:border-red-900/60 dark:bg-red-900/20 dark:text-red-300"
                >
                  {{ t("admin.settings.defaultTierGroupIds.loadFailed") }}
                </div>
                <TierGroupChainEditor
                  v-model="defaultTierGroupIdsValue"
                  :groups="openAITierGroupOptions"
                  :placeholder-text="t('keys.selectTierGroup')"
                  :search-placeholder-text="t('keys.searchGroup')"
                  :add-button-text="t('keys.addTierGroup')"
                  :hint-text="t('admin.settings.defaultTierGroupIds.hint')"
                />
                <div class="flex justify-end border-t border-gray-100 pt-4 dark:border-dark-700">
                  <button
                    type="button"
                    @click="saveDefaultTierGroupIds"
                    :disabled="defaultTierGroupIdsSaving || defaultTierGroupIdsLoadFailed"
                    class="btn btn-primary btn-sm"
                  >
                    <svg
                      v-if="defaultTierGroupIdsSaving"
                      class="mr-1 h-4 w-4 animate-spin"
                      fill="none"
                      viewBox="0 0 24 24"
                    >
                    ...
                    {{ defaultTierGroupIdsSaving ? t("common.saving") : t("common.save") }}
                  </button>
                </div>
              </template>
            </div>
...
// Default Tier Group IDs 状态
const defaultTierGroupIdsLoading = ref(true);
const defaultTierGroupIdsSaving = ref(false);
const defaultTierGroupIdsLoadFailed = ref(false);
const defaultTierGroupIdsValue = ref<number[]>([]);
const allGroupsForTier = ref<AdminGroup[]>([]);
const openAITierGroupOptions = computed(() =>
  allGroupsForTier.value
...
// Default Tier Group IDs 方法
async function loadDefaultTierGroupIds() {
  defaultTierGroupIdsLoading.value = true;
  defaultTierGroupIdsLoadFailed.value = false;
  try {
    const [res, groups] = await Promise.all([
      adminAPI.settings.getDefaultTierGroupIds(),
      adminAPI.groups.getAll(),
    ]);
    defaultTierGroupIdsValue.value = res.ids ?? [];
    allGroupsForTier.value = groups;
  } catch (error: unknown) {
    defaultTierGroupIdsLoadFailed.value = true;
    appStore.showError(
      extractApiErrorMessage(error, t("admin.settings.defaultTierGroupIds.loadFailed")),
    );
  } finally {
    defaultTierGroupIdsLoading.value = false;
  }
}

async function saveDefaultTierGroupIds() {
  if (defaultTierGroupIdsLoadFailed.value) {
    appStore.showError(t("admin.settings.defaultTierGroupIds.loadFailed"));
    return;
  }
  defaultTierGroupIdsSaving.value = true;
  try {
    const res = await adminAPI.settings.updateDefaultTierGroupIds(defaultTierGroupIdsValue.value);
    defaultTierGroupIdsValue.value = res.ids ?? [];
    appStore.showSuccess(t("admin.settings.defaultTierGroupIds.saved"));
  } catch (error: unknown) {
    appStore.showError(
      extractApiErrorMessage(error, t("admin.settings.defaultTierGroupIds.saveFailed")),
    );
  } finally {
    defaultTierGroupIdsSaving.value = false;
  }
}
